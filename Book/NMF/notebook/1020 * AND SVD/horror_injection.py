#!/usr/bin/env python3
# build_pair_bias_pos5_neg1__HORROR_ONLY.py
# Generate pair-injection CSVs ONLY for pairs that include the target genre ("Horror").
#  - positives: ALL books containing BOTH Horror and G → rated 5
...
TARGET_GENRE  = "Horror"
# <<< explicitly skip these Horror pairs:
SKIP_PARTNERS = {"Fantasy", "Adventure", "Adult"}
...
# ========== GENERATOR (Horror-only, pos=5) ==========
def run_for_horror_pos5(df: pd.DataFrame, base_start_uid: int):
    ...
    summary_txt = out_dir / "summary__HORROR_ONLY.txt"
    summary_csv = out_dir / "summary__HORROR_ONLY.csv"
    pairs_overview_csv = out_dir / "pairs_overview__HORROR_ONLY.csv"
    missing_pairs_csv  = out_dir / "missing_pairs__HORROR_ONLY.csv"
    ...
    # Only genres paired with Horror, excluding Horror itself and the explicit skips
    ...
    for g in partner_genres:
        ...
        # positives: books containing BOTH Horror and g
    ...
    with open(summary_txt, "a", encoding="utf-8") as log:
        ...
        log.write("="*80 + "\n")
        log.write(f"Grand total injected rows (Horror pairs, pos=5): {total_injected:,}\n")
        log.write(f"Pairs overview: {pairs_overview_csv}\n")
        log.write(f"Missing pairs: {missing_pairs_csv}\n\n")

    print(f"✅ Done for Horror-only pairs. Out: {out_dir}")
...
def main():
    ...
    run_for_horror_pos5(df, base_start_uid)

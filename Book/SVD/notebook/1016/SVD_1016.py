...
# Baseline on ORIGINAL, then per injected file produce Top-K recs for original users.
...
SINGLE_INJ_ROOT = Path("/home/moshtasa/Research/phd-svd-recsys/SVD/Book/result/rec/top_re/1016/Single_Injection")

# Output folder
RESULTS_ROOT = Path("/home/moshtasa/Research/phd-svd-recsys/SVD/Book/result/rec/top_re/1016/SVD_pair")
(RESULTS_ROOT / "5").mkdir(parents=True, exist_ok=True)

# ========= SETTINGS =========
TOP_N_LIST = [15, 25, 35]
N_FILTER = {2, 4, 6}   # only run these n-user variants
...
def scan_single_genre_pos5(root: Path):
    """
    Return POS=5 single-genre files under Single_Injection/, filtered to N in {2,4,6}.
...
def main():
    start = time.time()
    now("=== SVD (poison-only) — SINGLE GENRE (POS=5 ONLY, N ∈ {2,4,6}) ===")

    # Baseline
    now("🔰 Training baseline on ORIGINAL…")
    orig_df = load_df(ORIGINAL_PATH)
    original_users = set(orig_df[USER_COL].unique())
    now(f"📄 ORIGINAL — users={len(original_users):,}, items={orig_df[BOOK_COL].nunique():,}, rows={len(orig_df):,}")
    try:
        orig_map = create_genre_mapping(orig_df)
        svd_base, ts_base = train_svd(orig_df)
        now("🧠 Baseline SVD trained. Generating baseline Top-K…")

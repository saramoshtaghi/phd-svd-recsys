#!/usr/bin/env python3
# NMF_1216_mobile_original_and_injected.py
# Train & evaluate NMF on ORIGINAL + injected Mobile App datasets
...
RESULTS_ROOT = Path(
    "/home/moshtasa/Research/phd-svd-recsys/Mobile/SVD/result/rec/top_re/1216/NMF"
)
...
NMF_PARAMS = dict(
    n_factors=8,
    n_epochs=200,
    biased=False,        # IMPORTANT: pure NMF
    reg_pu=0.06,
    reg_qi=0.06,
    random_state=42,
    verbose=False,
...
def main():
    start = time.time()
    now("=== NMF — MOBILE APPS (ORIGINAL + Injected) ===")
    ...
    now(f"\n🏁 Finished all Mobile NMF runs (~{hrs:.2f} h)")

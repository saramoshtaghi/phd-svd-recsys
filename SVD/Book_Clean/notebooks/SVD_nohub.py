...
def load_dataset(file_path):
    ...
    # ensure numeric types for Surprise raw IDs
...
def create_genre_mapping(df):
    # map each book_id -> primary genre token (or "Unknown")
    mapping = {}
    for _, row in df.iterrows():
        ...
        if "," in g:
            ...
            primary = g.strip()
        mapping[bid] = primary if primary and primary.lower() != "nan" else "Unknown"
    return mapping
...
def generate_and_save_recommendations(df, original_users, genre_mapping, svd, base_name, results_dir):
    ...
    Save one CSV per N: {base_name}_{N}recommendation.csv
    """
        ...
        for n in TOP_N_LIST:
            ...
            for rank, (bid, est) in enumerate(topn, start=1):
                per_topn_rows[n].append({
                    ...
                    "est_score": est,
                    "rank": rank,
                    "genre": genre_mapping.get(int(bid), "Unknown")
                })

    # write files
    for n, rows in per_topn_rows.items():
        out_df = pd.DataFrame(rows, columns=["user_id", "book_id", "est_score", "rank", "genre"])
...
for filename in synthetic_files:
    ...
    # Train SVD on (original + synthetic) combined file (it already includes original + synthetic)
    svd_model = train_svd(synthetic_df)

    # Base output name is filename without extension (e.g., Romance_25k)
    base_name = os.path.splitext(os.path.basename(file_path))[0]

    # Save per-user top-N rec CSVs with requested naming pattern
    generate_and_save_recommendations(
        df=synthetic_df,

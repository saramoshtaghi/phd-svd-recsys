#!/usr/bin/env python3
# build_pair_bias_pos5_neg1__FANTASY_ONLY.py
# Generate pair-injection CSVs ONLY for pairs that include the target genre ("Fantasy").
#  - positives: ALL books containing BOTH Fantasy and G → rated 5
...
# <<< only build pairs that include this genre
TARGET_GENRE  = "Fantasy"
# <<< explicitly skip these Fantasy pairs:
SKIP_PARTNERS = {"Adult", "Adventure"}
...
# ========== GENERATOR (Fantasy-only, pos=5) ==========
def run_for_fantasy_pos5(df: pd.DataFrame, base_start_uid: int):
    ...
    summary_txt = out_dir / "summary__FANTASY_ONLY.txt"
    summary_csv = out_dir / "summary__FANTASY_ONLY.csv"
    pairs_overview_csv = out_dir / "pairs_overview__FANTASY_ONLY.csv"
    missing_pairs_csv  = out_dir / "missing_pairs__FANTASY_ONLY.csv"
    ...
    # Only genres paired with Fantasy, excluding Fantasy itself and the explicit skips
    partner_genres = [g for g in GENRES if g != TARGET_GENRE and g not in SKIP_PARTNERS]
    pair_index = 0

    for g in partner_genres:
        g1, g2 = TARGET_GENRE, g

        # positives: books containing BOTH Fantasy and g
    ...
    with open(summary_txt, "a", encoding="utf-8") as log:
        ...
        log.write("="*80 + "\n")
        log.write(f"Grand total injected rows (Fantasy pairs, pos=5): {total_injected:,}\n")
        log.write(f"Pairs overview: {pairs_overview_csv}\n")
        log.write(f"Missing pairs: {missing_pairs_csv}\n\n")

    print(f"✅ Done for Fantasy-only pairs. Out: {out_dir}")
...
def main():
    ...
    run_for_fantasy_pos5(df, base_start_uid)

...
# ========= NEW HELPER: CHECK IF JOB IS ALREADY COMPLETED =========
def is_job_completed(base_name: str, out_dir: Path) -> bool:
    """
    Returns True if ALL Top-N output files already exist for this base_name.
    """
    return all((out_dir / f"{base_name}_{n}recommendation.csv").exists() for n in TOP_N_LIST)
...
def main():
    ...
    for i, job in enumerate(jobs, 1):
        ...
        if is_job_completed(base_name, out_dir):
            now(f"\n⏭️ [{i}/{len(jobs)}] POS=5 {n_users}u — {genre.replace('_',' ')}")
            now(f"   File: {fp.name}")
            now("   All Top-N output files already exist. Skipping.")
            continue
